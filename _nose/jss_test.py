...
from jss import *
from jss.exceptions import JSSError, GetError
...
class JSSTestBase(object):
    
    jss = None  # type: Optional[JSS]
    jss_url = 'https://localhost:8443'  # type: str
    jss_user = 'admin'  # type: str
    jss_password = 'password'  # type: str
    jss_ssl_verify = False  # type: bool
    ...
    def setup_class(cls):
        cls.jss = JSS(
            url=cls.jss_url,
            user=cls.jss_user,
            password=cls.jss_password,
            ssl_verify=cls.jss_ssl_verify)
...
class TestJSS(JSSTestBase):
    ...
    def test_jss_get_error(self):
        assert_raises(JSSError, self.jss.get, '/donkey-tacos')

    def test_jss_get(self):
        policy = self.jss.Policy(1)
        assert_is_instance(policy, ElementTree.Element)

    def test_JSS_post(self):
        new_policy = Policy(self.jss, TESTPOLICY)
        new_policy.save()
